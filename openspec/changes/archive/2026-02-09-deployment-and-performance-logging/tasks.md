...
- [x] 6.7 在 `app/data/baostock.py` 的 `fetch_daily()` 添加细粒度日志（连接池等待、API 调用分步计时）
- [x] 6.8 优化 `app/data/pool.py` 的 `acquire()` 方法（队列为空时立即创建会话，避免 30 秒超时等待）
- [ ] 6.9 测试批量同步日志（验证日志格式和内容）
