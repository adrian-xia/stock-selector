...
- [ ] 3.1 在 `app/main.py` 添加信号处理器（监听 SIGTERM 和 SIGINT）
- [ ] 3.2 实现优雅关闭逻辑（停止接受新任务，等待运行中的任务完成）
- [ ] 3.3 实现超时强制停止（30 秒超时后强制关闭）
- [ ] 3.4 添加关闭日志（记录关闭开始、等待任务、关闭完成）
- [ ] 3.5 测试优雅关闭（验证任务完成后再停止，超时后强制停止）
...
- [ ] 5.5 实现 `--skip-integrity-check` 命令行参数（跳过检查）
...
- [ ] 6.8 测试补齐命令（验证跳过已有数据、只补齐缺失数据、速率限制）

## 7. 数据初始化向导实现

- [ ] 7.1 创建 `scripts/init_data.py` 初始化向导脚本
- [ ] 7.2 实现数据库状态检测（检查 stock_daily 表是否已有数据）
- [ ] 7.3 实现交互式选项（1 年 / 3 年 / 自定义范围，使用 input() 获取用户输入）
- [ ] 7.4 实现日期范围计算（1 年 ≈ 250 交易日，3 年 ≈ 750 交易日）
- [ ] 7.5 实现初始化流程编排（步骤 1: 股票列表 → 步骤 2: 交易日历 → 步骤 3: 日线数据 → 步骤 4: 技术指标）
- [ ] 7.6 实现步骤 1：同步股票列表（调用 manager.sync_stock_list()）
- [ ] 7.7 实现步骤 2：同步交易日历（调用 manager.sync_trade_calendar(start_date, end_date)）
- [ ] 7.8 实现步骤 3：同步日线数据（调用 batch_sync_daily，显示进度）
- [ ] 7.9 实现步骤 4：计算技术指标（调用 compute_all_stocks，显示进度）
- [ ] 7.10 添加进度显示（每 1000 只股票显示一次进度和预计剩余时间）
- [ ] 7.11 添加确认提示（数据库已有数据时提示是否继续）
- [ ] 7.12 添加大数据范围警告（自定义范围 >5 年时提示可能耗时数小时）
- [ ] 7.13 测试初始化向导（验证完整的初始化流程，测试各种数据范围选项）
...
- [ ] 9.1 测试打包流程（打包 → 解压 → 验证文件完整性）
- [ ] 9.2 测试 Linux 服务管理（安装 → 启动 → 状态查询 → 停止 → 卸载）
- [ ] 9.3 测试 macOS 服务管理（安装 → 启动 → 状态查询 → 停止 → 卸载）
- [ ] 9.4 测试前台启动模式（验证日志输出到 stdout）
- [ ] 9.5 测试优雅关闭（验证任务完成后再停止，超时后强制停止）
- [ ] 9.6 测试启动时数据完整性检查（模拟缺失数据，验证自动补齐）
- [ ] 9.7 测试 --skip-integrity-check 参数（验证跳过检查）
- [ ] 9.8 测试手动补齐命令（指定日期范围、验证跳过已有数据）
- [ ] 9.9 测试数据初始化向导（1 年 / 3 年 / 自定义范围）
- [ ] 9.10 测试完整部署流程（打包 → 传输 → 解压 → 配置 → 初始化 → 安装服务 → 启动）
- [ ] 9.11 验证日志输出（确保所有关键步骤都有日志记录）
- [ ] 9.12 验证错误处理（测试各种异常场景，确保错误提示清晰）
