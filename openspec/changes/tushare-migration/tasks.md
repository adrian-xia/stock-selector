...
- [x] 12.1 更新 `app/data/cli.py` — 新增 init-tushare 命令（全量初始化：stock_basic → trade_cal → 逐日 daily → fina → moneyflow → 指数/板块 → 技术指标）
- [x] 12.2 新增 `raw_sync_progress` 表追踪原始数据拉取进度，支持断点续传
...
- [x] 13.1 补全 `docs/tushare/` 下缺失的接口文档（从官网获取指数/板块/扩展接口文档）
