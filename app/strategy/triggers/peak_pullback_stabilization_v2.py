"""Trigger 策略：高位回落企稳。

趋势组信号，主升浪后深度回调，缩量企稳后放量小阳二次启动。
返回 list[StrategySignal]，含置信度。
"""

from datetime import date

import pandas as pd

from app.strategy.base import BaseStrategyV2, SignalGroup, StrategyRole, StrategySignal


class PeakPullbackStabilizationTriggerV2(BaseStrategyV2):
    """高位回落企稳 Trigger：主升浪后深度调整，缩量企稳后放量小阳二次启动。"""

    name = "peak-pullback-stabilization-trigger-v2"
    display_name = "高位回落企稳"
    role = StrategyRole.TRIGGER
    signal_group = SignalGroup.TREND
    description = "前期主升浪后高位回落，缩量企稳后放量小阳二次启动"
    default_params = {
        # Layer A：前期主升浪
        "min_peak_rise_pct": 20.0,
        "ma_tolerance": 0.03,
        # Layer B：高位回落
        "min_pullback_pct": 10.0,
        "max_pullback_pct": 35.0,
        # Layer C：缩量企稳
        "max_vol_ratio": 0.8,
        "ma5_band": 0.025,
        # Layer D：今日放量启动
        "min_pct_chg": 0.5,
        "max_pct_chg": 7.0,
        "min_signal_vol_ratio": 1.2,
    }
    ai_rating = 7.47  # 三模型均分

    async def execute(
        self,
        df: pd.DataFrame,
        target_date: date,
    ) -> list[StrategySignal]:
        """执行高位回落企稳检测。

        Returns:
            list[StrategySignal]，命中股票列表
        """
        p = self.params

        close = df.get("close", pd.Series(dtype=float)).fillna(0)
        open_ = df.get("open", pd.Series(dtype=float)).fillna(0)
        vol = df.get("vol", pd.Series(dtype=float)).fillna(0)
        pct_chg = df.get("pct_chg", pd.Series(dtype=float)).fillna(0)
        vol_ratio = df.get("vol_ratio", pd.Series(dtype=float)).fillna(0)
        ma5 = df.get("ma5", pd.Series(dtype=float)).fillna(0)
        ma5_prev = df.get("ma5_prev", pd.Series(dtype=float)).fillna(0)
        ma20 = df.get("ma20", pd.Series(dtype=float)).fillna(0)
        ma60 = df.get("ma60", pd.Series(dtype=float)).fillna(0)
        vol_ma5 = df.get("vol_ma5", pd.Series(dtype=float)).fillna(0)
        vol_ma10 = df.get("vol_ma10", pd.Series(dtype=float)).fillna(0)
        high_60 = df.get("high_60", pd.Series(dtype=float)).fillna(0)

        # 回落幅度
        pullback_pct = (high_60 - close) / high_60.replace(0, float("nan")) * 100
        pullback_pct = pullback_pct.fillna(0)

        # Layer A：前期主升浪
        layer_a = (
            (high_60 > 0)
            & (high_60 > ma60 * (1 + p["min_peak_rise_pct"] / 100.0))
            & (close >= ma60 * (1 - p["ma_tolerance"]))
            & (vol > 0)
        )

        # Layer B：高位回落确认
        layer_b = (
            (pullback_pct >= p["min_pullback_pct"])
            & (pullback_pct <= p["max_pullback_pct"])
            & (close < ma20)
        )

        # Layer C：缩量企稳
        vol_ma10_safe = vol_ma10.replace(0, float("nan"))
        recent_shrink = (vol_ma5 < vol_ma10_safe * p["max_vol_ratio"]).fillna(False)

        ma5_safe = ma5.replace(0, float("nan"))
        price_stable = ((close - ma5_safe).abs() / ma5_safe < p["ma5_band"]).fillna(
            False
        )

        ma5_rising = (ma5_prev > 0) & (ma5 >= ma5_prev)

        layer_c = recent_shrink & price_stable & ma5_rising & (pct_chg > -9.5)

        # Layer D：今日放量小阳启动
        layer_d = (
            (close > open_)
            & (pct_chg >= p["min_pct_chg"])
            & (pct_chg <= p["max_pct_chg"])
            & (vol_ratio >= p["min_signal_vol_ratio"])
        )

        signal_mask = layer_a & layer_b & layer_c & layer_d

        signals = []
        for ts_code in df.loc[signal_mask, "ts_code"]:
            signals.append(
                StrategySignal(
                    ts_code=ts_code,
                    confidence=1.0,
                )
            )

        return signals
