...
| §3 数据库表设计与 ETL | ✅ V1 已实施（P0+P1+P2+P3+P4+P5 建表） | **新增 raw 原始数据层**：90 张 raw_tushare_* 表（P0 6张 + P1 10张 + P2 10张 + P3 18张 + P4 8张 + P5 48张），两阶段 ETL（API → raw → 业务表）；**P0+P1+P2+P3 完整实施**（含 ETL 和数据同步），**P4-P5 仅建表**（V2 再实施 ETL 和数据同步）|
...
- **合计：90 张 raw_tushare_* 表**（P0+P1+P2+P3 完整实施含 ETL 和数据同步，P4-P5 仅建表和 ORM 模型）

**V1 删除的表：** `raw_baostock_daily`, `raw_akshare_daily`（已切换到 Tushare）

**V2 待实施：** P4-P5 数据同步和 ETL、所有数据校验测试
...
**后续计划：**
- V2 Phase 1：实施 P4-P5 扩展数据同步和 ETL
