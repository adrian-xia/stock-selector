...
| §3 模块8-新闻舆情 | ✅ V2 已实施 | 东方财富/淘股吧/雪球采集 + Gemini 情感分析 + 盘后链路步骤 3.9 + 前端新闻仪表盘 |
| §3 模块9-高手跟投 | V2 再加 | — |
...
| §3.6 P5 扩展数据原始表 | ✅ V1 已实施 | **已建表**：48 张 raw_tushare_* 表（涵盖股本、停复牌、两融、大宗交易、股东、质押、回购、涨跌停、龙虎榜、热榜、港股通、筹码分布等）+ ORM 模型 + fetch 方法；**已实施核心 ETL**：2 个 transform 清洗函数（suspend_d、limit_list_d）+ 2 张业务表（suspend_info、limit_list_daily）+ 约 20 个 DataManager sync_raw 方法（日频/周频/月频/静态/季度）+ etl_suspend/etl_limit_list + sync_p5_core 聚合方法 + 盘后链路步骤 3.8 集成；**剩余 28 张补充表**：Change 11 范围 |
...
| 任务 | 原优先级 | 调整 |
|:---|:---|:---|
| A1 参数优化 | P1 | → **V2 已实施**：网格搜索 + 遗传算法，优化任务管理，前端参数优化页面 |
...
- DataManager 方法：约 20 个 sync_raw 方法（日频 13 张 + 周频/月频 + 静态/季度）+ etl_suspend/etl_limit_list + sync_p5_core 聚合方法
- 盘后链路集成：步骤 3.8，按频率分组调度（日频/周频/月频/静态/季度），失败不阻断后续链路
- 剩余 28 张补充表：Change 11 范围
