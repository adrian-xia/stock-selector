...
| §3 数据库表设计与 ETL | ✅ V1 已实施（P0+P1+P2） | **新增 raw 原始数据层**：26 张 raw_tushare_* 表（P0 6张 + P1 10张 + P2 10张），两阶段 ETL（API → raw → 业务表）|
...
| §3.4 P3-P5 扩展原始表 | V2 再加 | 指数、板块、扩展数据（共 30+ 张表）|
| §3.5 数据校验测试 | V2 再加 | **所有数据校验测试（P0/P1/P2/P3/P4/P5）均在 V2 实施**，因为需要实际数据才能验证 |
...
**V2 待实施的表：** P2 数据同步和 ETL、P3 指数（16 张）、P4 板块（8 张）、P5 扩展数据（10+ 张）
...
**迁移范围：** P0 基础行情 + P1 财务数据 + P2 资金流向（部分）
...
**实施成果：**
- 新增 26 张 raw_tushare_* 表（P0 6张 + P1 10张 + P2 10张）
...
- `app/models/raw.py` - 26 张 raw_tushare_* 表 ORM 模型
