...
| §3 数据库表设计与 ETL | ✅ V1 已实施（P0+P1+P2+P3+P4+P5 建表） | **新增 raw 原始数据层**：90 张 raw_tushare_* 表（P0 6张 + P1 10张 + P2 10张 + P3 18张 + P4 8张 + P5 48张），两阶段 ETL（API → raw → 业务表）；**P0+P1 完整实施**（含 ETL 和数据同步），**P2-P5 仅建表**（V2 再实施 ETL 和数据同步）|
...
| §3.3 P2 资金流向原始表 | ⚠️ V1 部分实施 | **已建表**：10 张 raw_tushare_* 表（moneyflow, moneyflow_dc, moneyflow_ths, moneyflow_hsgt, moneyflow_ind_ths, moneyflow_cnt_ths, moneyflow_ind_dc, moneyflow_mkt_dc, top_list, top_inst）+ ORM 模型 + fetch 方法；**V2 再加**：ETL 函数、DataManager 数据同步方法 |
| §3.4 P3 指数数据原始表 | ✅ V1 已实施（仅建表） | **已建表**：18 张 raw_tushare_* 表（index_basic, index_daily, index_weekly, index_monthly, index_dailybasic, index_weight, index_member_all, index_classify, sw_daily, ci_index_member, ci_daily, index_factor_pro, tdx_index, tdx_daily, tdx_member, index_global, daily_info, sz_daily_info）+ ORM 模型 + fetch 方法；**V2 再加**：ETL 函数、DataManager 数据同步方法、业务表（index_daily, index_technical_daily） |
| §3.5 P4 板块数据原始表 | ✅ V1 已实施（仅建表） | **已建表**：8 张 raw_tushare_* 表（ths_index, ths_daily, ths_member, dc_index, dc_member, dc_hot_new, tdx_index, tdx_member）+ ORM 模型 + fetch 方法；**V2 再加**：ETL 函数、DataManager 数据同步方法、业务表（concept_index, concept_daily, concept_member, concept_technical_daily） |
| §3.6 P5 扩展数据原始表 | ✅ V1 已实施（仅建表） | **已建表**：48 张 raw_tushare_* 表（涵盖股本、停复牌、两融、大宗交易、股东、质押、回购、涨跌停、龙虎榜、热榜、港股通、筹码分布等）+ ORM 模型 + fetch 方法；**V2 再加**：ETL 函数、DataManager 数据同步方法 |
...
- **合计：90 张 raw_tushare_* 表**（P0+P1 完整实施含 ETL 和数据同步，P2-P5 仅建表和 ORM 模型）

**V1 删除的表：** `raw_baostock_daily`, `raw_akshare_daily`（已切换到 Tushare）

**V2 待实施：** P2-P5 数据同步和 ETL、所有数据校验测试
...
**迁移范围：** P0 基础行情 + P1 财务数据 + P2-P5 扩展数据（仅建表）
...
- `app/models/raw.py` - 90 张 raw_tushare_* 表 ORM 模型（P0+P1 完整实施，P2-P5 仅建表）
...
**P2 资金流向数据：**
- ⚠️ V1 仅建表和 ORM 模型，未实施数据同步和 ETL
- 原因：V1 优先保证核心功能（P0+P1），P2 留待 V2 实施
- 影响：资金流向策略暂时无法使用
...
**P2 资金流向数据同步：**
- ETL 清洗函数：`transform_tushare_moneyflow`, `transform_tushare_top_list`
- DataManager 方法：`sync_raw_moneyflow`, `etl_moneyflow`
- 数据校验测试：`test_p2_data_validation.py`

**P3 指数数据同步：**
- ETL 清洗函数：`transform_tushare_index_*`
- DataManager 方法：`sync_raw_index`, `etl_index`
- 业务表：`index_daily`, `index_technical_daily`
- 数据校验测试：`test_p3_data_validation.py`

**P4 板块数据同步：**
- ETL 清洗函数：`transform_tushare_concept_*`
- DataManager 方法：`sync_raw_concept`, `etl_concept`
- 业务表：`concept_index`, `concept_daily`, `concept_member`, `concept_technical_daily`
- 数据校验测试：`test_p4_data_validation.py`

**P5 扩展数据同步：**
- ETL 清洗函数：根据业务需求逐步实施
- DataManager 方法：根据业务需求逐步实施
- 数据校验测试：`test_p5_data_validation.py`
...
**性能优化：**
- 使用 PostgreSQL COPY 协议替代 INSERT（速度提升 10 倍）
- 全量导入前删除索引，导入后重建（速度提升 3-5 倍）
- TimescaleDB 超表和压缩（节省 90% 空间）
...
**后续计划：**
- V2 Phase 1：完成 P2 资金流向数据同步和 ETL
- V2 Phase 2：实施 P3-P5 扩展数据
- V2 Phase 3：补全所有数据校验测试
- V2 Phase 4：性能优化（COPY 协议、TimescaleDB）
