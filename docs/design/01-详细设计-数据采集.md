...
> **日期：** 2026-02-16
> **关联任务：** Task 0.1
> **状态：** Implemented (P0 + P1 + P2)
>
> **V1 实施范围：** Tushare Pro 单一数据源 + raw 原始数据层 + ETL 清洗层。已实施 P0（基础行情）、P1（财务数据）和 P2（资金流向，仅建表）。
...
# 命令行参数
uv run python -m scripts.init_data --range 1year
uv run python -m scripts.init_data --range 3years
uv run python -m scripts.init_data --start 2023-01-01 --end 2026-02-17
```
...
| **P2 资金流向** | 10 张 | ⚠️ 部分完成 | raw 表 + ORM 模型 + fetch 方法（V1 仅建表，数据同步和 ETL 留待 V2） |
| **P3 指数数据** | 16 张 | ⏸️ V2 再加 | 待实施 |
| **P4 板块数据** | 8 张 | ⏸️ V2 再加 | 待实施 |
| **P5 扩展数据** | 10+ 张 | ⏸️ V2 再加 | 待实施 |

**数据库现状：**
- 业务表：12 张（stock_basic, stock_daily, stock_technical_daily, trade_calendar, stock_sync_progress 等）
- raw 层表：26 张（P0 6张 + P1 10张 + P2 10张）

**P2 实施说明：**
- ✅ 已完成：10 张 raw_tushare_* 表创建、ORM 模型定义、TushareClient fetch_raw_* 方法
- ⏸️ 待实施（V2）：ETL 转换函数、DataManager 数据同步方法、数据校验测试
- 策略：V1 先建表和接口，为 V2 资金流向分析功能做准备
...
| raw 原始数据层 | ✅ 完成 | 26 张 raw_tushare_* 表，保留 API 原始格式 |
| 两阶段 ETL | ✅ 完成 | API → raw 表 → 业务表（P0+P1 已实施） |
...
| 数据初始化向导 | ✅ 完成 | 交互式引导首次数据初始化（1年/3年/自定义） |
