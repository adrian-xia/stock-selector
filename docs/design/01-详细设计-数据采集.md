...
> **状态：** Draft
>
> **V1 实施范围：** 见 `99-实施范围-V1与V2划分.md` §二。V1 去掉 raw 中转层、故障切换状态机、TimescaleDB 分区、断点续传、数据质量报告系统。

---

## 1. 模块概述

本模块负责从 BaoStock、AKShare、Tushare 等外部数据源获取股票行情、财务数据和榜单数据，经过标准化清洗（ETL）后，存储到系统数据库中。

**核心挑战：**
- **数据源异构：** 不同源的字段名、时间格式、复权方式不一致。
- **稳定性差异：** 免费源（BaoStock）可能偶尔超时或缺数据。
- **数据量大：** 分钟线数据每日增量巨大，需考虑写入性能。

---

## 2. 类图设计 (Class Diagram)

采用 **策略模式 (Strategy Pattern)** + **工厂模式** 来管理多数据源。
...
classDiagram
    class DataSourceClient {
        ...
        +fetch_minute(code: str, freq: str) -> List[Dict]
        +fetch_finance(code: str, report_date: str) -> Dict
        +health_check() -> bool
    }
    ...
    class DataManager {
        ...
        +sync_daily_all()
        +sync_minute_all()
        +switch_source(new_source: str)
    }

    DataSourceClient <|-- BaoStockClient
    DataSourceClient <|-- AKShareClient
    DataSourceClient <|-- TushareClient
    DataManager --> DataSourceClient
```
...
**设计原则：** 字段全部使用 `VARCHAR`，允许 NULL，不做任何转换，确保 100% 还原 API 返回值。

#### A. `raw_baostock_daily` (BaoStock 日线)
| 字段名 | 类型 | 示例值 | 说明 |
| :--- | :--- | :--- | :--- |
| id | SERIAL | 1 | 主键 |
| fetch_time | TIMESTAMP | 2024-01-01 16:00:00 | 采集时间 |
| code | VARCHAR | sh.600519 | 原始代码 |
| date | VARCHAR | 2024-01-01 | 日期 |
| open | VARCHAR | 1700.00 | 开盘价 |
| ... | ... | ... | high, low, close |
| adjustflag | VARCHAR | 3 | 3=不复权 |
| tradestatus | VARCHAR | 1 | 1=正常 |
| raw_json | JSONB | {...} | 完整行数据备份 |

#### B. `raw_akshare_daily` (AKShare 实时)
| 字段名 | 类型 | 示例值 | 说明 |
| :--- | :--- | :--- | :--- |
| 代码 | VARCHAR | 600519 | **中文键名** |
| 名称 | VARCHAR | 贵州茅台 | |
| 最新价 | VARCHAR | 1705.20 | |
| 涨跌幅 | VARCHAR | 0.52 | |
| ... | ... | ... | |
...
**设计原则：** 统一字段名、统一单位、统一精度 (`DECIMAL`)、统一代码格式 (`600519.SH`)。
...
- AKShare (`600519`) + 市场判断 → `600519.SH`
- Tushare (`600519.SH`) → 保持不变

**2. 数值类型转换**
- 空值处理：`""`, `"N/A"`, `"--"`, `"None"` → `NULL` (数据库)
- 精度控制：
  - 价格：`DECIMAL(10, 2)` (保留2位)
  - 涨跌幅：`DECIMAL(10, 4)` (保留4位，如 0.0521)
  - 成交量：`DECIMAL(20, 2)` (单位：手)
  - 成交额：`DECIMAL(20, 2)` (单位：千元)

**3. 复权处理 (关键!)**
- 策略回测必须使用**前复权 (Forward Adjusted)** 价格。
- **方案：**
  - 每日存储 **不复权 (Raw)** 价格到 `stock_daily`。
  - 每日存储 **复权因子 (Adj Factor)** 到 `adj_factor` 字段。
  - 回测时动态计算：`复权价 = 原始价 * 当日复权因子 / 最新复权因子`。

---

## 4. 多源故障切换状态机 (Failover FSM)

为了保证每日数据 100% 完整，设计如下状态机：

```mermaid
stateDiagram-v2
    [*] --> Primary(BaoStock)
    
    Primary(BaoStock) --> Success: 成功获取
    Primary(BaoStock) --> VerifyFailed: 数据完整性校验失败
    Primary(BaoStock) --> Timeout: API超时/报错
    
    VerifyFailed --> Secondary(AKShare): 触发切换
    Timeout --> Secondary(AKShare): 触发切换
    
    Secondary(AKShare) --> Success: 成功获取
    Secondary(AKShare) --> Tertiary(Tushare): 依然失败
