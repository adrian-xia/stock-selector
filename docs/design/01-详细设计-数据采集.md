...
> **日期：** 2026-02-16
> **关联任务：** Task 0.1
> **状态：** Implemented (P0 + P1 + P2 + P3 + P4 + P5)
>
> **V1 实施范围：** Tushare Pro 单一数据源 + raw 原始数据层 + ETL 清洗层。已实施 P0（基础行情）、P1（财务数据）、P2（资金流向）、P3（指数数据）、P4（板块数据）、P5（扩展数据），共 98 张 raw 表 + 完整 ETL 和数据同步。
...
|:---|:---|:---|:---|:---|
| 日线行情 (`stock_daily`) | Tushare Pro | 各股票上市日至今（最早可追溯至 1990-12-19） | 约 2500 万行（5000 只 x 平均 5000 个交易日） | 约 2-4 小时 |
| 财务指标 (`finance_indicator`) | Tushare Pro (VIP 接口) | 全量（各股票上市以来所有季报） | 约 50 万行（5000 只 x 平均 100 个季度） | 约 1-2 小时 |
| 复权因子 (`adj_factor`) | Tushare Pro | 各股票上市日至今 | 约 2500 万行 | 约 1-2 小时 |
| 交易日历 (`trade_calendar`) | Tushare Pro | 1990-01-01 至今 | 约 9000 行 | < 1 分钟 |

> **说明：**
> - V1 采用 Tushare Pro 单一数据源，按日期批量获取全市场数据，性能大幅提升
> - 总导入耗时预估 4-8 小时（受网络带宽和 Tushare QPS 限制影响）
> - 建议使用数据初始化向导 (`uv run python -m scripts.init_data`) 进行首次导入
> - V1 不包含分钟线数据（V2 再加）

#### 8.1.2 断点续传机制

##### `stock_sync_progress` 表 DDL

**V1 实际实施：** 使用 `stock_sync_progress` 表追踪每只股票的数据同步和指标计算进度。

```sql
CREATE TABLE stock_sync_progress (
    ts_code         VARCHAR(16)     PRIMARY KEY,    -- 股票代码，如 '600519.SH'
    status          VARCHAR(16)     NOT NULL        -- 'idle' | 'syncing' | 'computing' | 'failed' | 'delisted'
                    DEFAULT 'idle',
    data_date       DATE            NOT NULL        -- 数据已同步到的日期
                    DEFAULT '1900-01-01',
    indicator_date  DATE            NOT NULL        -- 指标已计算到的日期
                    DEFAULT '1900-01-01',
...
COMMENT ON TABLE stock_sync_progress IS '股票数据同步进度表（支持断点续传）';
COMMENT ON COLUMN stock_sync_progress.data_date IS '数据已同步到的日期（stock_daily）';
COMMENT ON COLUMN stock_sync_progress.indicator_date IS '技术指标已计算到的日期（technical_daily）';

CREATE INDEX idx_stock_sync_progress_status ON stock_sync_progress (status);
CREATE INDEX idx_stock_sync_progress_data_date ON stock_sync_progress (data_date);
```

**V1 新增：** `raw_sync_progress` 表追踪原始数据拉取进度（按表+同步键）。

```sql
CREATE TABLE raw_sync_progress (
    table_name      VARCHAR(64)     NOT NULL,       -- raw 表名，如 'raw_tushare_daily'
    sync_key        VARCHAR(32)     NOT NULL,       -- 同步键，如 '20260216'（日期）或 '20231231'（季度）
    status          VARCHAR(16)     NOT NULL        -- 'pending' | 'syncing' | 'completed' | 'failed'
                    DEFAULT 'pending',
    record_count    INTEGER         DEFAULT 0,      -- 已同步记录数
    synced_at       TIMESTAMP,                      -- 同步完成时间
    retry_count     INTEGER         DEFAULT 0,      -- 已重试次数
    last_retry_at   TIMESTAMP,                      -- 最近一次重试时间
    error_message   TEXT,                           -- 最近一次错误信息
    created_at      TIMESTAMP       NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMP       NOT NULL DEFAULT NOW(),
...
COMMENT ON TABLE raw_sync_progress IS '原始数据同步进度表（支持断点续传）';
COMMENT ON COLUMN raw_sync_progress.sync_key IS '同步键：日线数据用日期（YYYYMMDD），财务数据用季度（YYYYMMDD）';

CREATE INDEX idx_raw_sync_progress_status ON raw_sync_progress (table_name, status);
```

##### 断点续传逻辑

**V1 实际实施：** 基于 `stock_sync_progress` 表的断点续传机制。

```mermaid
flowchart TD
    A[启动数据同步] --> B[查询 stock_sync_progress 表]
    B --> C{是否存在未完成任务?}
    C -->|是| D[筛选 status NOT IN delisted,failed 且 data_date < target_date 的股票]
    C -->|否| E[初始化：为所有未退市股票插入记录]
    D --> F[按 data_date 确定每只股票的起始日期]
    E --> F
    F --> G[逐只股票拉取数据]
    G --> H[更新 status=syncing]
    H --> I[从 data_date+1 开始按批次拉取（365天/批）]
    I --> J{拉取成功?}
    J -->|是| K[批量写入 stock_daily]
    K --> L[更新 data_date 和 status=idle]
    L --> M[计算技术指标]
    M --> N[更新 indicator_date]
    J -->|否| O[记录 error_message, retry_count+1]
    O --> P{retry_count >= max_retry?}
    P -->|是| Q[status=failed, 跳过该股票]
    P -->|否| R[等待后重试]
    R --> I
    N --> S{还有下一只股票?}
    Q --> S
...
**关键特性：**
- 启动时自动恢复未完成同步（`status IN syncing,computing` → `idle`）
- 按 365 天/批分批处理，避免单次拉取数据量过大
- 失败自动重试（每日 20:00 重试任务）
- 退市股票自动过滤（`status=delisted`）
- Redis 分布式锁防并发
...
**V1 实际实施：** 使用数据初始化向导 (`scripts/init_data.py`) 进行首次导入。
...
class DataInitializer:
    ...
    async def run_full_import(self, start_date: date, end_date: date) -> dict:
        """
        执行全量导入主流程。
        ...
        # Step 1: 同步股票列表
        logger.info("Step 1/5: 同步股票列表")
        await self.manager.sync_stock_list()

        # Step 2: 同步交易日历
        logger.info("Step 2/5: 同步交易日历")
        await self.manager.sync_trade_calendar(
            start_date=start_date,
            end_date=end_date + timedelta(days=90)  # 额外获取未来 90 天
        )

        # Step 3: 获取交易日列表
        trading_dates = await self.manager.get_trade_calendar(start_date, end_date)
        logger.info(f"Step 3/5: 共 {len(trading_dates)} 个交易日需要同步")

        # Step 4: 逐日同步日线数据（按日期批量获取全市场数据）
        logger.info("Step 4/5: 同步日线数据")
        for i, trade_date in enumerate(trading_dates):
            logger.info(f"[{i + 1}/{len(trading_dates)}] 同步 {trade_date}")
            try:
                # 4.1 获取原始数据到 raw 表
                await self.manager.sync_raw_daily(trade_date)
                # 4.2 ETL 清洗到业务表
                await self.manager.etl_daily(trade_date)
                stats["success"] += 1
        ...
        # Step 5: 计算技术指标
        logger.info("Step 5/5: 计算技术指标")
        from app.data.indicator import compute_incremental
        for trade_date in trading_dates:
            await compute_incremental(
                self.session_factory,
                target_date=trade_date,
            )

        logger.info(f"全量导入完成：成功 {stats['success']} 天，失败 {stats['failed']} 天")
        return stats
```
...
**V1 实际配置（Tushare Pro 单一数据源）：**

| 配置项 | 类型 | 默认值 | 取值范围 | 说明 |
|:---|:---|:---|:---|:---|
| `TUSHARE_TOKEN` | str | `""` | - | Tushare Pro API Token（**必填**，需在 tushare.pro 注册获取） |
| `TUSHARE_RETRY_COUNT` | int | 3 | 1-10 | Tushare 请求重试次数 |
| `TUSHARE_RETRY_INTERVAL` | float | 1.0 | 0.5-30.0 | Tushare 重试间隔（秒） |
| `TUSHARE_QPS_LIMIT` | int | 400 | 1-500 | Tushare 每分钟最大请求数（默认 400，积分 >= 2000 可设为 500） |

**说明：**
- V1 移除了 BaoStock/AKShare 配置
- Tushare QPS 限制通过令牌桶算法实现（`TokenBucket`）
- 财务数据使用 VIP 接口按季度批量获取，无需额外配置
...
| 配置项 | 类型 | 默认值 | 取值范围 | 说明 |
|:---|:---|:---|:---|:---|
| `AUTO_UPDATE_ENABLED` | bool | `true` | true/false | 是否启用自动数据更新 |
| `AUTO_UPDATE_TRIGGER_TIME` | str | `"15:30"` | HH:MM | 盘后链路触发时间 |
| `AUTO_UPDATE_PROBE_INTERVAL` | int | 15 | 5-60 | 数据嗅探间隔（分钟） |
| `AUTO_UPDATE_PROBE_TIMEOUT` | str | `"18:00"` | HH:MM | 嗅探超时时间 |
| `SCHEDULER_TIMEZONE` | str | `"Asia/Shanghai"` | IANA 时区 | 定时任务时区 |

**说明：**
- V1 使用智能自动更新系统替代固定时间任务
- 包含数据嗅探机制，自动检测数据就绪状态
- 详见 `10-系统设计-定时任务调度.md` §3.4.1
...
| 配置项 | 类型 | 默认值 | 取值范围 | 说明 |
|:---|:---|:---|:---|:---|
| `DATA_INTEGRITY_CHECK_ENABLED` | bool | `true` | true/false | 是否启用启动时数据完整性检查 |
| `DATA_INTEGRITY_CHECK_DAYS` | int | 30 | 1-365 | 检查最近 N 天数据完整性 |
| `DATA_INTEGRITY_THRESHOLD` | float | 0.95 | 0.0-1.0 | 数据完整性阈值（95%），低于此值触发告警 |
| `SKIP_INTEGRITY_CHECK` | bool | `false` | true/false | 环境变量，跳过启动时检查（用于快速启动） |

**说明：**
- 启动时自动检查最近 30 天数据完整性
- 完整性低于 95% 时自动补齐缺失数据
- 可通过环境变量 `SKIP_INTEGRITY_CHECK=true` 跳过检查
...
| 配置项 | 类型 | 默认值 | 取值范围 | 说明 |
|:---|:---|:---|:---|:---|
| `ETL_BATCH_SIZE` | int | 5000 | 500-50000 | 批量写入每批行数 |
| `DATA_START_DATE` | str | `"2015-01-01"` | YYYY-MM-DD | 历史数据起始日期（默认 10 年） |

**说明：**
- V1 使用 PostgreSQL `ON CONFLICT DO UPDATE` 实现 UPSERT
- 批量写入使用 SQLAlchemy `insert().values()` 方法
- 不使用 COPY 协议（V2 再优化）
...
**说明：**
- V1 使用数据初始化向导 (`scripts/init_data.py`) 进行首次导入
- 支持交互式选择：1年/3年/自定义范围
- 按日期批量获取全市场数据，性能优于逐股票遍历
- 不包含分钟线数据（V2 再加）
...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://user:pass@localhost/stock_selector",
        description="PostgreSQL 数据库连接字符串"
    )

    # Tushare 配置
    tushare_token: str = Field(default="", description="Tushare Pro API Token")
    tushare_retry_count: int = Field(default=3, ge=1, le=10)
    tushare_retry_interval: float = Field(default=1.0, ge=0.5, le=30.0)
    tushare_qps_limit: int = Field(default=400, ge=1, le=500)
    ...
    # 数据配置
    data_start_date: str = Field(default="2015-01-01", description="历史数据起始日期")

    # 数据完整性检查配置
    data_integrity_check_enabled: bool = Field(default=True)
    data_integrity_check_days: int = Field(default=30, ge=1, le=365)
    data_integrity_threshold: float = Field(default=0.95, ge=0.0, le=1.0)

    # 自动更新配置
    auto_update_enabled: bool = Field(default=True)
    auto_update_trigger_time: str = Field(default="15:30")
    auto_update_probe_interval: int = Field(default=15, ge=5, le=60)
    auto_update_probe_timeout: str = Field(default="18:00")
...
```bash
# .env 文件
DATABASE_URL=postgresql+asyncpg://user:pass@localhost/stock_selector
...
### 13.1 Tushare 数据源迁移完成情况

| 阶段 | 表数量 | 实施状态 | 说明 |
|:---|:---:|:---|:---|
| **P0 基础行情** | 6 张 | ✅ 完成 | raw 表 + ORM 模型 + fetch 方法 + ETL 函数 + DataManager 方法 + 数据校验测试 |
| **P1 财务数据** | 10 张 | ✅ 完成 | raw 表 + ORM 模型 + fetch 方法（VIP 接口按季度批量获取）+ ETL 函数 + DataManager 方法 + 数据校验测试 |
| **P2 资金流向** | 10 张 | ✅ 完成 | raw 表 + ORM 模型 + fetch 方法 + ETL 函数 + DataManager 方法 + 数据同步 |
| **P3 指数数据** | 7 张 | ✅ 完成 | raw 表 + ORM 模型 + fetch 方法 + ETL 函数 + DataManager 方法 + 数据同步（含 idx_factor_pro） |
| **P4 板块数据** | 6 张 | ✅ 完成 | sync_concept_index/daily/member + ETL + 盘后链路集成 |
| **P5 扩展数据** | 34 张 | ✅ 完成 | raw 表 + ORM 模型 + fetch 方法 + DataManager 方法 + 数据同步（含 suspend_info/limit_list_daily ETL） |

**数据库现状：**
- 业务表：15 张（stocks, stock_daily, technical_daily, trade_calendar, stock_sync_progress, money_flow, dragon_tiger, index_basic, index_daily, index_weight, index_technical_daily, concept_index, concept_daily, concept_member, concept_technical_daily, suspend_info, limit_list_daily 等）
- raw 层表：98 张（P0 6张 + P1 10张 + P2 10张 + P3 18张 + P4 8张 + P5 48张，其中 34 张启用同步）

**P2-P5 实施说明：**
- ✅ P0-P5 全部已完成：raw 表创建、ORM 模型、fetch 方法、ETL 转换、DataManager 同步方法、盘后链路集成
- ✅ CLI `init-tushare` 支持 `--skip-p0` ~ `--skip-p5` 按优先级跳过
- ✅ 交互式向导 `scripts/init_data.py` 支持 P0-P5 全量同步

### 13.2 核心功能实施状态

| 功能模块 | 实施状态 | 说明 |
|:---|:---:|:---|
| 令牌桶限流 | ✅ 完成 | TokenBucket 实现，400 QPS 限制 |
| 按日期批量获取 | ✅ 完成 | 避免逐股票遍历，大幅提升性能 |
| raw 原始数据层 | ✅ 完成 | 98 张 raw_tushare_* 表，保留 API 原始格式 |
| 两阶段 ETL | ✅ 完成 | API → raw 表 → 业务表（P0-P5 全部已实施） |
| VIP 接口批量获取 | ✅ 完成 | 财务数据按季度批量获取，降级到标准接口 |
| 智能数据自动更新 | ✅ 完成 | 数据嗅探 + 智能重试 + 超时告警 |
| 断点续传机制 | ✅ 完成 | stock_sync_progress 表追踪进度，启动时自动恢复 |
| 数据完整性检查 | ✅ 完成 | 启动时检查最近 30 天数据，自动补齐缺失 |
| 数据初始化向导 | ✅ 完成 | 交互式引导首次数据初始化（P0-P5 全量同步），CLI init-tushare 9 步 |
| 优雅关闭 | ✅ 完成 | 等待运行中任务完成后关闭（30秒超时） |

### 13.3 测试覆盖情况

| 测试类型 | 覆盖范围 | 状态 |
|:---|:---|:---:|
| 单元测试 | TushareClient、ETL 函数、DataManager 方法 | ✅ 完成 |
| 集成测试 | P0 数据校验（6 个测试用例） | ✅ 完成 |
| 集成测试 | P1 数据校验（5 个测试用例） | ✅ 完成 |
| 集成测试 | P2 数据校验（5 个测试用例） | ⏸️ V2 再加 |
| 功能测试 | 数据初始化向导、优雅关闭、自动数据更新 | ✅ 完成 |
