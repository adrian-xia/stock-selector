...
- **Tushare 数据源** — 对接 Tushare Pro API，自动同步日线行情、财务指标、资金流向、指数数据、板块数据等，引入 raw 层作为数据缓冲，解耦数据获取和清洗逻辑；已实施 P0（基础行情 6 张表）、P1（财务数据 10 张表，使用 VIP 接口按季度批量获取）、P2（资金流向 10 张表）、P3（指数数据 18 张表，含日常同步和静态数据同步，idx_factor_pro 批量模式优化 API 配额）、P4（板块数据 8 张表，含盘后链路集成）和 P5（全部 48 张表数据同步 + 2 张业务表 ETL + 盘后链路集成，含 28 张补充表按频率分组同步）
- **高性能数据同步** — 按日期批量获取全市场数据（避免逐股票遍历），令牌桶限流（400 QPS，特殊接口独立限流桶：stk_auction/stk_factor/ccass_hold/limit_list_d），全链路性能日志支持瓶颈分析（数据同步、技术指标、缓存刷新、调度任务）；COPY 协议批量写入（10 倍提升），全量导入索引管理（3-5 倍提升），TimescaleDB 超表自动压缩（可选），连接池调优
