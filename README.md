...
- **高性能数据同步** — 按日期批量获取全市场数据（避免逐股票遍历），令牌桶限流（400 QPS，特殊接口独立限流桶），全链路性能日志支持瓶颈分析（数据同步、技术指标、缓存刷新、调度任务）；COPY 协议批量写入（10 倍提升），全量导入索引管理（3-5 倍提升），TimescaleDB 超表自动压缩（可选），连接池调优
