...
- **Tushare 数据源** — 对接 Tushare Pro API，自动同步日线行情、财务指标、资金流向、指数数据、板块数据等，引入 raw 层作为数据缓冲，解耦数据获取和清洗逻辑；已实施 P0（基础行情 6 张表）、P1（财务数据 10 张表，使用 VIP 接口按季度批量获取，含利润表/资产负债表/现金流量表全量字段）、P2（资金流向 10 张表）、P3（指数数据 18 张表，含日常同步和静态数据同步，idx_factor_pro 批量模式优化 API 配额）、P4（板块数据 8 张表，含盘后链路集成）和 P5（全部 48 张表数据同步 + 2 张业务表 ETL + 盘后链路集成，含 28 张补充表按频率分组同步）
...
│   ├── models/                 # ORM 模型（33 张业务表 + 99 张 raw 层表）
│   ├── data/                   # 数据采集模块
