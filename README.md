...
- **36 种选股策略** — 24 种技术面策略（趋势跟踪、震荡指标、量价分析、量价配合）+ 12 种基本面策略（估值、盈利、安全、综合评分），支持自由组合；策略注册制管理，盘后链路仅执行用户启用的策略，支持自定义参数；**V2 架构开发中**：策略角色分层（guard/scorer/tagger/trigger/confirmer），36 → 20 策略精简，市场状态自适应权重，详见 `docs/design/20-策略引擎V2-全新设计.md`
