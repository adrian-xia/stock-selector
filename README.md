...
- **Tushare 数据源** — 对接 Tushare Pro API，自动同步日线行情、财务指标、资金流向、指数数据、板块数据等，引入 raw 层作为数据缓冲，解耦数据获取和清洗逻辑；已实施 P0（基础行情 6 张表）、P1（财务数据 10 张表，使用 VIP 接口按季度批量获取）、P2（资金流向 10 张表）、P3（指数数据 18 张表，含日常同步和静态数据同步）、P4（板块数据 8 张表，含盘后链路集成）和 P5 核心（约 20 张表数据同步 + 2 张业务表 ETL + 盘后链路集成）
...
- **技术指标计算** — 自动计算 MA/MACD/KDJ/RSI/BOLL/ATR/WR/CCI/BIAS/OBV/唐奇安通道 等 29 个技术指标
- **20 种选股策略** — 16 种技术面策略（趋势跟踪、震荡指标、量价分析）+ 4 种基本面策略，支持自由组合
...
- **定时任务** — 盘后自动执行数据同步、指标计算、缓存刷新、策略筛选、AI 分析全链路
...
- **测试覆盖** — 单元测试覆盖全部 API 端点、策略引擎、回测引擎、数据源客户端、数据完整性检查、数据初始化向导、优雅关闭、自动数据更新、P2 资金流向 ETL、P3 指数数据 ETL、P4 板块数据 ETL；集成测试覆盖 P0-P5 数据校验（数据完整性、ETL 转换正确性、数据质量、跨表一致性、综合时间连续性和数据新鲜度）
