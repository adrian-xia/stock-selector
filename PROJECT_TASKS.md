...
> 更新日期：2026-03-06
> 当前状态：V4 StarMap Phase 4 完成，进入陪跑观察期（需持续半个月）；V2 策略引擎重构已全部落地
...
### 策略引擎
- ✅ V2 策略引擎：20 个 V2 策略（1 scorer + 5 confirmer + 2 guard + 2 tagger + 10 trigger）
- ✅ V2 Pipeline：Layer 0 SQL → Layer 1 质量底池 → Layer 2 trigger → Layer 3 多因子融合
- ✅ 盘后链路 / 工作台 / 策略配置页 / 全市场优化已全部切到 V2
- ✅ 废弃 V1 主执行链路与旧策略文件已删除，数据库残留策略与关联历史已物理清理
- ✅ V4 `volume-price-pattern` 独立保留
- ✅ 行业/市场筛选：110 个行业 + 4 个市场
- ✅ 动态加权：市场状态系数 × rolling_performance `[0.8, 1.2]`，再叠加 style bonus 与 confirmer additive bonus

### 回测与优化
- ✅ Backtrader 回测引擎（A 股佣金、涨跌停限制）
- ✅ 参数优化：网格搜索 + 遗传算法
- ✅ 全市场选股回放优化（MarketOptimizer，仅优化 V2 trigger，目标=命中率+盈亏比−回撤）
- ✅ 每周 cron 自动执行并应用最佳参数
...
| V2 策略补全（Doc-00） | 79 策略是文档愿景，当前生产主路径已收敛到 20 个 V2 策略；事件驱动与 StarMap 重叠 |
...
- **V3**：量价策略体系扩展完成，后续已并入 V2 策略引擎重构主线
- **V4**：StarMap 盘后投研系统 Phase 0-4 基本完成，已部署生产环境，进入陪跑观察期
- **当前阶段**：V4 陪跑观察中；V2 策略引擎、V2 优化器、V2 前端适配、旧 V1 清理均已完成
